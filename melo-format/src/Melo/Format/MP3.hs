...
mp3Metadata MP3{..} = H.fromList $ catMaybes
...
instance I.InfoReader MP3 where
  info mp3 = let FrameHeader{..} = frameHeader mp3 in I.Info {
  ...
  hLocate h =
    ...
    where
      findHeader pos = do
        ...
        bs <- L.hGetContents h
        if isRight $ bdecodeOrFail @FrameHeader bs then
          pure $ Just (fromIntegral pos)
        else pure Nothing
