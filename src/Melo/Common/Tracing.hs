...
class Tracing m where
  withSpan'' :: CallStack -> Text -> SpanArguments -> (Span -> m a) -> m a

withSpan :: (HasCallStack, Tracing m) => Text -> SpanArguments -> m a -> m a
withSpan n a m = withSpan'' callStack n a (const m)

withSpan' :: (HasCallStack, Tracing m) => Text -> SpanArguments -> (Span -> m a) -> m a
withSpan' = withSpan'' callStack
...
instance
  ...
  Tracing (t m)
  where
  withSpan'' cs n a m = liftWith (\run -> withSpan'' cs n a (run . m)) >>= restoreT . pure
...
withGlobalTracer m = ifM (liftIO $ lookupEnv "OTEL_EXPORTER_OTLP_TRACES_ENDPOINT" <&> isn't _Nothing)
  do
    bracket
      (liftIO Otel.initializeGlobalTracerProvider)
      Otel.shutdownTracerProvider $ const m
  do
    m
...
getTracer = getAppData @TracerWrapper >>= \case
...
instance Tracing (AppM IO IO) where
  withSpan'' cs n a m = do
    tracer <- getTracer
    Otel.inSpan'' tracer cs n a m
