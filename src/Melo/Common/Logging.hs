...
import Language.Haskell.TH.Syntax (Loc(..), Q, Exp, qLocation, liftString)
...
newtype LoggingC m a = LoggingC {
  runLoggingC :: m a
} deriving newtype (Applicative, Functor, Monad, MonadIO)

instance (Has (Lift IO) sig m, Algebra sig m) =>
  Algebra (Logging :+: sig) (LoggingC m) where
